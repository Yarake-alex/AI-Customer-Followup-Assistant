...
def delete_customer(customer_id: int, db: Session = Depends(get_db), _=Depends(verify_admin_key)):
    customer = db.query(Customer).filter(Customer.id == customer_id).first()
    if not customer:
        raise HTTPException(status_code=404, detail="客户不存在")
...
def ai_summary(customer_id: int, db: Session = Depends(get_db), _=Depends(verify_admin_key)):
    customer = db.query(Customer).filter(Customer.id == customer_id).first()
    if not customer:
        raise HTTPException(status_code=404, detail="客户不存在")
...
def ai_suggestion(customer_id: int, db: Session = Depends(get_db), _=Depends(verify_admin_key)):
    customer = db.query(Customer).filter(Customer.id == customer_id).first()
    if not customer:
        raise HTTPException(status_code=404, detail="客户不存在")
...
@app.post("/rag/upload")
async def upload_rag_document(file: UploadFile = File(...), db: Session = Depends(get_db), _=Depends(verify_admin_key)):
...
def delete_rag_document(filename: str, db: Session = Depends(get_db), _=Depends(verify_admin_key)):
    deleted_count = (
...
def clear_rag_documents(db: Session = Depends(get_db), _=Depends(verify_admin_key)):
...
def rag_ask(data: RagAsk, db: Session = Depends(get_db), _=Depends(verify_admin_key)):
...
def agent_analyze(data: AgentAnalyzeRequest, db: Session = Depends(get_db), _=Depends(verify_admin_key)):
